...
public class CubecraftIntegration extends BaseServerIntegration {
    ...
    private static final Pattern PARTY_INVITE = Pattern.compile("^You have received a party invite from (.+)\\.$");

    private static final Pattern KILL = Pattern.compile("^\\+3 Points$");
    ...
    public boolean supports(String host) {
        return host.equals("cubecraft.net") || host.endsWith(".cubecraft.net");
    }

    private final BooleanSetting partyInvites = new BooleanSetting("Party Invites", "party_invites", true);

    private final BooleanSetting killNotifications = new BooleanSetting("Kill Notifications", "kill_notifications", true);
    ...
    public void onChatMessage(String message) {
        ...
        if (partyMatcher != null && partyInvites.getValue()) {
            ...
            NotificationManager.push("[" + this.getName() + "]" + " Party invite", playerName + " invited you to a party", NotificationType.SOCIAL, 5000);
        }
        if (killMatcher != null && killNotifications.getValue()) {
            NotificationManager.push("[" + this.getName() + "] Kill", "You killed a player", NotificationType.INFO, 3000);
        }
    }
}